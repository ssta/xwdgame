...
public class TwitchBot {
  ...
  public final String getClientID() {
    if (this.clientID != null) {return this.clientID;} else {
    ...
    try {
      while ((line = this.reader.readLine()) != null && !stopped) {
        if (line.toLowerCase().startsWith("ping")) {
          ...
          if (msg_msg.startsWith(commandTrigger)) {onCommand(msg_user, msg_channel, msg_msg.substring(1));}
          ...
          if (p[1].equals("JOIN")) {userJoins(User.getUser(pd[0].substring(1)), Channel.getChannel(p[2], this));}
        } else if (line.contains(" PART ")) {
          String[] p = line.split(" ");
          String[] pd = line.split("!");
          if (p[1].equals("PART")) {userParts(User.getUser(pd[0].substring(1)), Channel.getChannel(p[2], this));}
          ...
          if (line.contains("This room is now in slow mode. You may send messages every")) {
            ...
            } else {LOGGER.log(Level.INFO, "> The room has been set to Subscribers Only!");}
          } else {
